...
namespace InstantPay.Application.Factory
{
    public class RechargeApiProviderService: IRechargeApiProviderService
    {
        ...
        public async Task<string> Process(string provider, string mobile, string amount, string orderId, string companyId, string Type, string Optional, string Optional1)
        {
            if (!_providers.TryGetValue(provider.ToLower(), out var repo))
                throw new Exception($"Provider [{provider}] not supported");

            return await repo.Recharge(mobile, amount, orderId, companyId, Type, Optional, Optional1);
        }
    }
}
