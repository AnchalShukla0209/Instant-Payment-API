...
namespace InstantPay.Application.Services
{
    public class JPBCashDeposit : IJPBCashDeposit
    {
        ...
        public JPBCashDeposit(AppDbContext context, IHttpClientFactory httpFactory, IConfiguration config, IHttpContextAccessor httpContextAccessor)
        {
            _context = context;
                                        ...
                                        try
                                        {
                                            if (parsed.ResponseCode == "00" && parsed.ResponseMessage.ToUpper() == "SUCCESS")
                                            {
                                                var ub = new Tbluserbalance
                                                {
                                                    ...
                                                    CrdrType = "DR", // Option B => credit
                                                    Remarks = $"AEPS CASH DEPOSIT TXN:{tx.TxnId}",
                                                    WlId = userData.Wlid,
