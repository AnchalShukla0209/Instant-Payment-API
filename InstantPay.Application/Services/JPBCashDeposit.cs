...
{
    public class JPBCashDeposit : IJPBCashDeposit
    {
        ...
        public JPBCashDeposit(AppDbContext context, IHttpClientFactory httpFactory, IConfiguration config, IHttpContextAccessor httpContextAccessor, IWalletService walletService)
        {
            _context = context;
        ...
        public async Task<CashDepositResponseDto> CashDeposit(CashDepositRequest model, CancellationToken cancellationToken = default)
        {
            if (model == null) throw new ArgumentNullException(nameof(model));
            if (string.IsNullOrWhiteSpace(model.Aadhaar)) return new CashDepositResponseDto { Success = false, ResponseMessage = "Aadhaar is required", ResponseCode = "33", accessToken = model.AccessToken, appIdentifierToken = model.AppIdentifierToken };
            if (string.IsNullOrWhiteSpace(model.FingerprintXml)) return new CashDepositResponseDto { Success = false, ResponseMessage = "PID XML or PidBase64 required", ResponseCode = "33", accessToken = model.AccessToken, appIdentifierToken = model.AppIdentifierToken };
            if (model.Amount <= 0) return new CashDepositResponseDto { Success = false, ResponseMessage = "Amount is required", ResponseCode = "33", accessToken = model.AccessToken, appIdentifierToken = model.AppIdentifierToken };
            ...
            {
                return new CashDepositResponseDto { Success = false, ResponseMessage = "finger print data is missing", ResponseCode = "33", accessToken = accessToken, appIdentifierToken = appIdToken };
            }
            ...
            var body = new
            {
                transaction = new
                {
                    ...
                    currency = 356,
                    invoice = uid,
                    method = new { type = 217, subType = 550 },
                    mode = 2,
                    captureMethod = 1,
                    livemode = "true",
            ...
            var tx = new TransactionDetail
            {
                ...
                TxnId = uid,
                ServiceName = "AEPS",
                OperatorName = "AEPS_CASH_DEPOSIT",
                OpId = null,
            ...
            {
                await LogApiAsync(apiUrl, "POST", "33", dbEx.Message, deviceInfoJson, requestBody, null, "AEPS", "CashDeposit");
            }
            ...
            while (attempts < maxAttempts)
            {
                ...
                try
                {
                    await LogApiAsync(apiUrl, "POST", "INIT", "", deviceInfoJson, requestBody, null, "AEPS", "CashDeposit");
                    ...
                    await LogApiAsync(apiUrl, "POST", resp.IsSuccessStatusCode ? "00" : resp.StatusCode.ToString(),
                                      null, deviceInfoJson + "x-appid-token:" + appIdToken + ",x-app-access-token:" + accessToken+", traceid: "+ traceId + "", requestBody, responseContent, "AEPS", "CashDeposit");
                    ...
                    if (!isSessionExpired)
                    {
                        if (resp.IsSuccessStatusCode)
                        {
                            ...
                            try
                            {
                                var parsed = JsonConvert.DeserializeObject<CashDepositResponseDto>(responseContent);
                                ...
                                try
                                {
                                    if (parsed?.ResponseData?.transaction != null)
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            {
                                                var (_, actualNewBal, _) = await _walletService.DebitAsync(
                                                    userData.Id, userData.Name + "-" + userData.Phone,
                                                    model.Amount, cost, rtComm, tds,
                                                    "AEPS_CASH_DEPOSIT",
                                                    $"AEPS CASH DEPOSIT TXN:{tx.TxnId}",
                                        ...
                                        {
                                            await LogApiAsync(apiUrl, "DB", "33", ubEx.Message, deviceInfoJson+", x-traceid:"+ traceId + "", requestBody, responseContent, "AEPS", "CashDeposit_TblUserBalance");
                                        }
                                    }
                                }
                            ...
                            catch (Exception parseEx)
                            {
                                ...
                                return new CashDepositResponseDto
                                {
                                    Success = true,
                                    ResponseCode = "00",
                ...
                {
                    await LogApiAsync(apiUrl, "POST", "33", ex.Message, deviceInfoJson, requestBody, null, "AEPS", "CashDeposit");
