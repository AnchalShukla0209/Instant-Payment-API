...
/// <summary>
/// Verification service for the Client (White-Label admin / tblWlUsers) module.
/// Mirrors ClientUserVerificationService but persists against TblWlUsers, and
/// uses an isolated cache-key namespace so challenges/proofs never collide with
/// the ClientUser (tblUsers) verification flow.
/// </summary>
public sealed class ClientVerificationService : IClientVerificationService
{
    ...
    /// when the client record is created via CreateOrUpdateClient.
    /// </summary>
    private async Task PersistVerificationAsync(int clientId, string type, string value, string? verifiedName)
    {
        ...
        var user = await _context.TblWlUsers.FirstOrDefaultAsync(u => u.Id == clientId);
        if (user == null)
            return;
    ...
    {
        var cooldownKey = $"client-verification:cooldown:{type}:{value}";
    ...
    private static string ChallengeKey(Guid id) => $"client-verification:challenge:{id:N}";
    private static string ProofKey(string token) => $"client-verification:proof:{token}";
