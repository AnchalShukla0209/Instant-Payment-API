...
public class PPIWalletService : IPPIWalletService
{
    ...
    public async Task<PPILoadWalletResponse> LoadWalletAsync(PPILoadWalletRequest request)
    {
        try
        {
            ...
            decimal rtComm = await GetCommissionFromPlanAsync(planId, amount, "RT");
            decimal totalDebit = amount + rtComm;
            ...
            // Make the API call
            var response = await _httpClient.PostAsync($"{_baseUrl}v1/wallet/loadwallet", content);
            ...
            {
                _logger.LogError("PPI Load Wallet API returned error status: {StatusCode}, Response: {Response}", 
                ...
                root.TryGetProperty("resultMessage", out var resultMessage))
            {
                ...
                if (resultCode.GetString() == "2000" && resultStatus != "FAILED")
                {
                    tx.Status = "Success";
                    tx.Brid = txnId;
                    await _context.SaveChangesAsync();
